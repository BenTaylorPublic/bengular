function build(bengularOptions) {
    let buildStartTime = new Date();
    const myFunctions = require("./my-functions");
    const options = getBuildOptions(bengularOptions);
    ...
    allTasks.push({
        ...
        fn: async function (buildOptions) {
            return myFunctions.eslint(buildOptions, spawnSync, process.stdout, bengularOptions);
        }
    });
    ...
    allTasks.push({
        ...
        fn: async function (buildOptions) {
            ...
            await myFunctions.bundle(module.name, module.entryDir, outDir, shared);
            return 0;
        }
    });
    ...
    allTasks.push({
        ...
        fn: async function (buildOptions) {
            ...
            for (const bundle of bundles) {
                await myFunctions.bundle(bundle.name, bundle.entryDir, outDir, shared);
            }
        ...
        fn: async function (buildOptions) {
            ...
            for (let i = 0; i < jsFiles.length; i++) {
                ...
                if (result.error != null) {
                    console.log(`Error on bundle ${docFilePath}`);
    ...
    function getBuildOptions() {
        ...
        if (bengularOptions.environment === "prod") {
            result.buildOptions.environment = "prod";
            console.log(`Build type: prod`);
        } else if (bengularOptions.environment === "dev") {
            result.buildOptions.environment = "dev";
            if (bengularOptions.changedFile != null) {
                //It's a grunt watch reload, so choose the correct file type
                const filepath = bengularOptions.changedFile;
                ...
                if (filepath.includes("/pages/")) {
                    ...
                    console.log(`Build type: dev ${result.buildOptions.devBuildType}, module: ${result.buildOptions.devBuildModule}`);
                } else {
                    //All
                    result.buildOptions.devBuildType = `all ${extension}`;
                    console.log(`Build type: dev ${result.buildOptions.devBuildType}`);
                }
            }
            if (result.buildOptions.devBuildType === "all") {
                console.log(`Build type: dev`);
            }
        }
    ...
    async function runTasks() {
        ...
        for (let i = 0; i < allTasks.length; i++) {
            ...
            if (options.verbose) {
                console.log(`${taskWillRun ? "+" : "-"} ${task.name}`);
            }
        }
        ...
        for (let i = 0; i < tasksToRun.length; i++) {
            ...
            if (!options.verbose) {
                ...
                //Verbose
                console.log(`${myFunctions.getBar(tasksDone, amountOfTasks)}: ${task.name}`);
            }
        ...
        myFunctions.clearLine();
        console.log(`${myFunctions.getBar(tasksDone, amountOfTasks)} Built in ${seconds}s`);
    }
