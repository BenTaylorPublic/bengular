...
function getBengularJson(arguments) {
    ...
    for (let i = 0; i < arguments.length; i++) {
        ...
        if (arg === "--verbose") {
            ...
            const port = Number(arg.replace("--port=", ""));
            if (port <= 0 || isNaN(port)) {
                myFunctions.log(`Invalid port ${port}`, colors.red);
            } else {
                bengularJson.port = port;
            }
        }
    }
    return {
...
const dev = {
    ...
    fn: function (arguments) {
        ...
        if (arguments.length >= 2 && isHelp(arguments[1])) {
            ...
            myFunctions.log("--verbose \tEnable verbose logging");
            myFunctions.log("--port=4200 \tSet the port to host on");
        } else {
