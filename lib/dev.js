    ...
    let server = http.createServer(function (req, res) {
        ...
        if (req.url.includes("/bengular-build-error")) {
            ...
            req.on("end", () => {
                ...
                if (json.error != null) {
                    latestBuildErrorOrNull = cleanUpErrorString(json.error);
                }
                res.writeHead(200);
                res.end(null);
                socketio.sockets.emit("refresh", {
        ...
        if (req.url.includes(".html") || !req.url.includes(".")) {
            if (latestBuildErrorOrNull != null) {
                ...
                result = `<!DOCTYPE html><html lang="en"><head><script src=\"socket.io.js\"></script><script defer>${myFunctions.getAutoRefreshScript()}</script></head><body><pre>${latestBuildErrorOrNull}</pre></body></html>`;
                res.writeHead(200);
                res.end(result);
                return;
            }
